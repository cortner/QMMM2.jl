...
Rmax = 75.0 * r0
atmax0,_ = SWqmmm.edge_config(Rmax, sw)
atmax, _ = SWqmmm.edge_config(Rmax, sw)
X0 = positions(atmax0)
...
for n = length(RQM):-1:1, imm = length(ships):-1:1
   ...
   at = deepcopy(atmax)
   xc = at["xcore"]
   r = [ norm(x[1:2] - xc[1:2]) for x in positions(at) ]
   Iqm = findall(r .< RQM[n])
...
println(" Rqm[Å] |" * "|  E [eV] 2-norm  ∞-norm  "^3)
println("-"^85)
for n =  1:length(RQM)
   @printf("  %5.1f |", RQM[n])
